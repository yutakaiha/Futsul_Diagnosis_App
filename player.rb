...
class Player
  ...
  def fit_position(result)
    fit_position = self.diagnosis(result)
    self.position_explanation(fit_position)
  end

  def position_explanation(fit_position)
    if fit_position.size >= 2
      puts "あなたに適性と思われるポジションが複数存在しました。"
      fit_position.each.with_index(1) do |position, i|
      ...
      puts <<-EOS
      あなたの適性ポジションは#{fit_position}です。
      現時点で自身のポジションが#{fit_position}なら、それは天性と言っても過言ではありません。
      一つのポジションを極め、圧倒的なスペシャリストを目指しましょう。
      あなたの専門的なスキルがここぞというタイミングで発揮されれば、チームのピンチを何度も救うことになるでしょう。
      いや、私は#{fit_position}なんてやったことないよ〜という人でも、自身の知られざる才能を開花させる
      チャンスと思って、一度#{fit_position}にチャレンジしてみてはいかがでしょうか？
